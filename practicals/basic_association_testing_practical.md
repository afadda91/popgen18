...
# Try to perform that on SNP1. This can be done as follows in R:

# Get the allelic counts (make sure you understand what is going on here)
...
# Make vector that indicates which of the numbers in the count vector are
# from cases (1) and which are from controls (0)
status<-rep(1:0,3)

# Make vector that show which of the numbers in the count vector are
# from AA (0), Aa (1) and aa (2):
geno<-rep(0:2,each=2)
